...
ggplot(payday, aes(x=loan, y=well.being)) + geom_jitter(aes(col = factor(loan))) + geom_hline(data = mean.wellbeing.loan, aes(yintercept = mean.well, col = factor(loan))) + geom_smooth(data = payday, mapping = aes(x=loan, y=well.being), method = "lm", se=FALSE)
...
m.wellbeing.loan.SES <- lm(well.being ~ SES + loan, data = payday)
m.wellbeing.loan.SES.combo <- lm(well.being ~ SES * loan, data = payday)
