...
According to the result, adding $SES$ as an independent varaible will make the linear regression model better.
...
anova(m.titanic.1st.by.age.binom, test="Chisq")
We can say: "The effect of age upon survival is significant, $\chi^2(214)=270$, $p=.0001$"

Because things are not linear any more, we do not use SSE and $F$-ratios. But the likelihood concept still applies---we are choosing the logistic regression curve that is most likely to have generated the data---the curve from which the data are most likely to have been sampled
...
```{r}
m.adverse.by.loan.factor.SES.binom <- glm(adverse.credit.event ~ loan * factor(SES), family=binomial, data=payday)
m.adverse.by.loan.SES.binom.emm <- summary(emmeans(m.adverse.by.loan.factor.SES.binom, ~loan+SES, type="response"))
...
ggplot(m.adverse.by.loan.SES.binom.emm, aes(x=SES, col=factor(loan), fill=factor(loan), y=prob, ymin=asymp.LCL, ymax=asymp.UCL)) + 
    geom_ribbon(alpha=0.5) + geom_line()+ geom_jitter(data=payday, mapping=aes(y=adverse.credit.event, x=SES, col=factor(loan), ymin=NULL, ymax=NULL), alpha = 0.2, width = 0.5, height = 0.06) + 
    ...
    labs(x="SES", y="Probability of Adverse Credit Event", subtitle="Dots are individuals who experienced adverse credit event or not.\n+s are the probability of adverse credit event according to Socio Economic Status.\nThe line and ribbon are the fit of a logistic regression model and its 95% CI") + guides(col=guide_legend(title="Loan"), fill=guide_legend(title="Loan")) + theme(plot.title = element_text(hjust = 0.5)) +
    ylim(0,1)
```
